...
\begin{abstract}
  ...
  Multiclass multilabel classification refers to the task of attributing multiple labels to examples via predictions. Current models formulate a reduction of that multilabel setting into either multiple binary classifications or multiclass classification, allowing for cross-entropy loss computation. Empirically, these methods have been reported to achieve good performance on different metrics (F1 score, Recall, Precision, etc.). Theoretically though, the multiple binary classification reduction does not model interactions between label propensities and the multiclass classification reduction does not allow to predict varying amount of labels per example.

  We propose a loss function based on the F1 score that is tailored to realistic multilabel settings. \gab{optional: [More generally, we show that any confusion matrix metric can be formulated with a smooth surrogate.]} We evaluate the loss function on different text and image datasets, and with a variety of metrics, to account for the complexity of mutilabel classification evaluation. We embed sigmoidF1 in a classification head that is attached to state-of-the-art pretrained neural networks MobileNetV2 and DistilBERT.
