...
\begin{abstract}
  Multilabel classification is a common task when learning from text, image or sound, but few models are tailored towards learning abstract descriptors of an entire text/image/sound. Current solutions tend to re-frame the problem as an instance of the multiclass domain, which allow the use of variations of cross-entropy or multinomial logit losses. These produce unit-interval results that require a sophisticated thresholding regime (at training time or after training), when the number of groundtruth labels varies over each example. As a decomposable F1 score surrogate, /sigmoidF1/ can implicitly deal with thresholding. The proposed method yields higher macro F1 results on several datasets of different modes, thus enabling higher level abstractions in the embedding space to be directly predictive of abstract labels in the output space.
% \begin{itemize}
