...
Among the three datasets used for benchmarking ML-NET cite:multitaskLabel, a cancer hallmark dataset is of sub-entity unilabel nature cite:cancerHallmarks (available at https://www.cl.cam.ac.uk/&sim;sb895/HoC.html): the research clearly describe a process of annotating several expressions within paper abstracts. The remaining two for chemical exposure cite:chemExposure (available at https://figshare.com/articles/Corpus_and_Software/4668229) and diagnosis codes assigment cite:diagnosisCode (available at https://physionet.org/works/ICD9CodingofDischargeSummaries), seem to fit to the entity wide multilabel definition but have a strong hierarchical nature. 
...
Cancer can be described according to its complexity with different principles, named hallmarks cite:cancerHallmarks. A corpus of 1580 PubMed abstracts are manually annotated for 10 hallmarks. This is a sub-entity labelling task and will therefore not be used here.
