...
\begin{equation}
S(u)=\frac{1}{1+\exp (-\beta (u + \eta))}
\end{equation}

Note that \(\lim \beta \rightarrow \infty S(u)\) corresponds to the step function used in Equation \ref{eq:conf} \todo{wrong equatino number in ref}. The confusion matrix entries then become
...
\label{sec:org7b96cce}
\section{datasets}
\label{sec:orgace9825}
