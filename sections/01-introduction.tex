...
\hvk{my proposal: 1) multi-class classification problems are hard because of
...
problem is the Single-Instance 102 Multilabel Prediction for Unknown Label
counts. 2) Commonly, for these problems, metrics like AUROC, F1 are used.
...
abstract concepts. 3) We are trying to bridge this gap by proposing soft
versions of these metrics. (give examples in tables). 4) In this paper we
focus on a soft version of the F1 and propose to use it directly as a loss in
a NN. We show in experiments that this loss improves results}
