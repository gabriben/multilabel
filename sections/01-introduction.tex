...
\textit{Arxiv} \hvk{ref} as \textit{Machine Learning (cs.LG)},
...
These three examples in the domains of
...
sparsity~\citep[see, e.g.,][]{focalLoss,tencent} \daan{need to explain
sparsity here more.}. Existing measures for multilabel prediction can be
...
label counts. For example, a cross-entropy loss surrogate \daan{You never
really introduced the idea of surrogates, did you? Why is it needed? What are
the limitations? etc.} would penalize for the number of wrongly predicted
...
gradient descent. \todo{check again with the authors if I can't get inspired
from their work}. Recently, a similar work has been proposed to train a
