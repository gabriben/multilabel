...
\mdr{An important shortcoming shared by both classes of solutions is XXX.}

\paragraph{Our proposed solution to FIMPUL tasks}
We propose an alternative type of solution that is aimed at \mdr{balancing, the F1 intuition}. Our proposed solution, \mdr{NAME}, is neither of the fit-algorithm-to-data type nor of the fit-data-to-algorithm type. 
Instead, it is best characterized as a an example of a \emph{design-algorithm-for-data} approach.
Briefly, \mdr{[description here]}

\if0
%%% we do not need the next two paragraphs, certainly not in the introduction.
...
We propose \mdr{confusion matrix metrics framework}.
