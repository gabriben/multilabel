...
we propose a loss framework based on confusion matrix metrics as losses. they have in common that they require step functions that are not decomposable for SGD: We propose a soft thresholding method with a sigmoid function to remedy this.

Certain classification problems do not yet have a designed solution to solve them. One of these is the problem of multilabel classification with unknown number of labels. This is particularly fitting to the new models which are able to learn abstract labnels from abstract representations.
Some of these metrics-as-losses can be particularly useful for tackling problems

Note that previous litterature has the tendency to reframe the problem . 
Note that the literature often brings back the problem to a multiclass multilabel

As neural network models are able to learn increasingly more abstract representations via deeper networks, representation learning and self-supervision~\citep[see, e.g.,][]{SS,Rep}, it is reasonable to expect that they also get better at predicting more abstract labels. 
...
These higher-level concepts can be mutually inclusive (not mutually exclusive). For example, a scientific journal can be tagged as \emph{machine learning} and \emph{economics}, a movie can be tagged as \emph{romance} and \emph{comedy}, a song can be tagged as \emph{jazz} and \emph{soul}. Among the candidate class labels, more than one of the candidate labels can be correct and the number of correct labels is unknown a priori. This is the classification problem of interest along this paper. It can be regarded as a classical supervised learning task. However it is not straightforward to place that problem within the supervised learning nomenclature.
