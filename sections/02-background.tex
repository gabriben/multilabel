...
They can be divided into two groups, \emph{fit-data-to-algorithm} solutions (a.k.a. \emph{problem tranformation} \todo{cite}) and \emph{fit-algorithm-to-data} solutions (a.k.a.\ \emph{algorithm adaptation}) solutions~\cite{multilabelReview} and will be discussed in \S\ref{section:background:fitdata} and \S\ref{section:background:fitalgorithm} respectively.
We then discuss shortcomings of these solutions in \S\ref{section:background:limitations} and propose a new solution framework in \S\ref{section:background:solution}. 
...
\daan{I rewrote this paragraph, please check for correctness.}
...
In the case of \emph{fit-algorithm-to-data} solutions, elements of the learning algorithm are changed to deal with non-mutually exclusive classes, such as the back propagation procedure or the tasks \daan{tasks is vague here. learning tasks is better, but can we be even more precise?}. 
...
label counts. \daan{retrieval: huh? change to just: "number"? Now that I think of it, it is also not a constraint, right?}
For example, a \daan{"adapted"?} cross-entropy loss surrogate would penalize for the number of wrongly predicted
...
With our design-algorithm-for-data approach, we design loss functions that address FIMPUL problems specifically, \daan{by optimizing for wrong labels and wrong label count simultaneously.}
...
at hand. \daan{This feels very out of place here. I can imagine it might be a good way to start of 2.4?}

The proposed design-algorithm-for-data method is thus an alternative to fit-data-to-algorithm and fit-algorithm-to-data. design-algorithm-for-data uses metric as losses, allows for dynamic thresholding and implicitly deals with label counts and label predictions. \daan{In the next section, we will detail this approach to derive \emph{sigmoidF1} loss as a surrogate to optimize for the F1 metric.}
...
\daan{Either related work or the old-sections folder ;-).}
