...
\mdr{One sentence reminder as to why we want confusion matrix metrics}


For a class of multilayer perceptron \(\mathcal{F}(\cdot ; \Theta): \mathcal{X} \rightarrow \mathcal{Y}\), we consider a special case, where \(\mathbf{x} = \{x_1, \ldots, x_n\}\). Each observation is attributed one or more classes out of a label set \(\mathbf{l} = \{l_1, \ldots, l_c\}\). Labels \(y_{i}^{j}\) are available for each observation \(i\) and class \(j\). 
