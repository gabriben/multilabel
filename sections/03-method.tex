...
Given the four confusion matrix quadrants, we can generate further metrics like precision and recall (see Table \ref{confusion-matrix}). \daan{The sentence before should be in the proposed confusion matrix primer, the one after makes sense here.} However none of these metrics are decomposable due to the hard thresholding which is in effect a step function (see Figure \ref{fig:sigmoid}).
...
\begin{prop}
  Steepness: $f'(u) >> 0 \quad \forall \; u \in [-s, s]$, with $s$ the saturation bound
\end{prop}

Outside the saturation bounds $[-s, s]$, the derivative should be significantly significantly higher than zero in order to facilitate gradient descent and backpropagation.

Note that the upper and lower limits of $f(u)$ are sometimes $[-1,1]$ instead of $[0,1]$ along this paper. It is easy to show that the desirable properties above are still valid
...
This method though has the advantage of resulting in a linear loss function that avoids the concept of thresholding altogether and is trivial to decompose for stochastic gradient descent.
...
The metrics described below are a result of a survey of different common practices for measuring accuracy of multilabel prediction. When true positives and false positives are used, recall that \(t p=\sum_{i \in Y^{+}} \mathds{1}_{\mathbf{p_i} \geq t}\) and \(f p=\sum_{i \in Y^{-}} \mathds{1}_{\mathbf{p_i} \geq t}\), and thus a threshold \(t\) must be set. When \(t = 0.5\), as is commonly done \todo{add source}, a risk remains that a lot of examples remain without predictions.
