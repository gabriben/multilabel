...
For the same class of learning algorithms defined in the previous section \(\mathcal{F}(\cdot ; \Theta): \mathcal{X} \rightarrow \mathcal{Y}\), we consider a special case, where \(\mathbf{x} = \{x_1, \ldots, x_n\}\) \daan{Is this what is special about our case?}. Each observation is assigned one or more classes out of a set of labels \(\mathbf{l} = \{l_1, \ldots, l_c\}\). Labels \(y_{i}^{j}\) are available for each observation \(i\) and class \(j\) \daan{Available feels vague. Sharpen.}. 
...
Let \(tp\), \(fp\), \(fn\), \(tn\) be number of true positives, false positives, false negatives and true negatives respectively. It is necessary to define a bound \(b\) \daan{"threshold" for consistency?}, at which a prediction is dichotomized:
%
\begin{equation}
\label{eq:conf}
...
A first remedy is to define \emph{unbounded} confusion matrix entries \daan{by replacing the dichotomized predictions with prediction probabilities. This way, }
