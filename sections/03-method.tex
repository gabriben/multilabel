...
\daan{Please check.}
In evaluating multiclass and multilabel classification problems, we can consider both micro-averaged and macro-averaged metrics. A micro-averaged metric computes the metric for each class which is then averaged over all classes. Micro-averaging is biased by class frequency, whereas macro-averaging takes all classes as equally important. Note that in the formulation above and the ones that follow, scores are calculated for a single class, therefore the sum is implicit over all examples \(\sum_i\). In this formulation, we thus compute macro-averages and consider all classes as equally important.
...
Given the four confusion matrix quadrants, we can generate further metrics like precision and recall (see Table \ref{tab:confusion-matrix}). However none of these metrics are decomposable due to the hard thresholding which is in effect a step function (see Figure \ref{fig:sigmoid}). \daan{Thought for more page filling. Include the normal confusion matrix quadrant. Should be a quick hack from the fancy table.}
